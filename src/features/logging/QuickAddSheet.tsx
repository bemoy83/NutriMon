...
import { useRecentProducts, useFrequentProducts, useProductSearch, useInvalidateProducts } from './useProducts'
...
export default function QuickAddSheet({ logDate, loggedAt, onClose, onAdded }: Props) {
  ...
  function addPendingItem(product: Product) {
  ...
  function updateQuantity(productId: string, quantity: number) {
    if (quantity <= 0) {
      setPendingItems((prev) => prev.filter((i) => i.product.id !== productId))
    } else {
      setPendingItems((prev) =>
    ...
    <SheetContainer onClose={onClose} title="Add meal">
        ...
        <div className="px-4 py-2 bg-indigo-950 border-b border-slate-700">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-white text-sm font-semibold">
              {pendingItems.reduce((sum, i) => sum + Math.round(i.quantity * i.product.calories), 0)} kcal
            </span>
          </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {getActiveProducts().map((product) => {
          const pending = pendingItems.find((i) => i.product.id === product.id)
          return (
            <ProductRow
              key={product.id}
              product={product}
              quantity={pending?.quantity ?? null}
              onAdd={() => addPendingItem(product)}
