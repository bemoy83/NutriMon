...
export function mapBattleLogEntry(row: BattleLogEntryRow): BattleLogEntry {
  return {
    ...
    actor: row.actor,
    action: row.action,
    firstActor: row.first_actor ?? null,
    playerInitiative: row.player_initiative ?? null,
    opponentInitiative: row.opponent_initiative ?? null,
    playerAction: row.player_action ?? null,
