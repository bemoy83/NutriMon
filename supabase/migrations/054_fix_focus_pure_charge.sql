-- 054_fix_focus_pure_charge.sql
-- Re-stamps submit_battle_action with the correct pure-charge Focus behavior
-- that was intended by 047_focus_multi_hit_rework.sql.
--
-- Root cause: migration 047 was applied to the DB while the file still contained
-- the old deal-damage Focus mechanic, then the file was updated to the pure-charge
-- version before committing. Supabase recorded 047 as applied so the corrected
-- function body was never pushed. This migration closes that drift.
--
-- Changes vs. the live 026/027 function:
--   Focus turn  — no upfront damage; sets next_attack_bonus = 1.0; target = null.
--   Attack turn — if next_attack_bonus > 0: fires 3-hit focused attack (3 × 75%
--                 base damage, per-hit crit at +50% relative chance, capped 35%).
--   Special     — enemy-only 4th action slot (damage_boost + multi_hit subtypes).
--   Meters      — player_special_meter / enemy_special_meter columns (always 0
--                 until the meter system ships; added with IF NOT EXISTS).

-- ─── 1. Ensure special-meter columns exist (idempotent) ──────────────────────
...
-- ─── 2. Replace submit_battle_action ─────────────────────────────────────────
...
begin
  ...
  -- For multi_hit / focused attack: these pre-rolled values are not used;
  -- each hit rolls its crit independently.
    ...
    case v_actor_action
      ...
      when 'attack' then
        ...
        if v_act_consumed_nab then
          -- ── Focused Attack: 3 hits × 75% base damage ───────────────────
          -- Each hit crits independently at +50% relative chance, capped 35%.
      ...
      when 'focus' then
        -- ── Pure charge turn ────────────────────────────────────────────────
        -- No upfront damage. Arms the next Attack as a 3-hit focused combo.
        -- The actor is exposed this turn: incoming attacks deal 1.3× damage.
        -- next_attack_bonus = 1.0 is the sentinel checked by the attack branch.
      ...
      when 'special' then
        -- ── Special action (enemy-only) ─────────────────────────────────────
        -- Sub-dispatch on special_action.type from battle_opponents.
        -- Consumes momentum boost identically to attack.
        -- Focus charge (next_attack_bonus) is reserved for standard Attack.
        v_act_consumed_mb  := v_act_momentum_boost > 0;
        ...
        case v_special_type
          ...
          when 'damage_boost' then
            -- Compute base damage (crit baked via v_act_crit_mult), then apply multiplier.
          ...
          when 'multi_hit' then
            -- Strikes N times; each hit crits independently.
            -- params.hits: 2–5 recommended.
            -- params.damage_fraction: per-hit fraction of base (e.g. 3 × 0.40 ≈ 120% total).
