-- Migration 076: Remove skill unlock level gate from submit_battle_action
--
-- Skill unlocks are permanent milestones tied to companion.level (live).
-- The backend was gating on snapshot.level (frozen at daily prep), which lags
-- behind companion.level whenever the player levels up mid-session. This caused
-- counter_stance and overdrive to silently fail for players who had legitimately
-- earned the unlock. The UI already prevents picking locked skills; the backend
-- check adds no security value in a PvE game.
