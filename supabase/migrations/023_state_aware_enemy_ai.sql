-- 023_state_aware_enemy_ai.sql
-- Extracts enemy action selection into a reusable helper and adds three
-- reactive rules on top of the static action_weights table:
--
--   Rule 1 — Desperation  : enemy HP ≤ 25% of max  → force attack (100/0/0)
--   Rule 2 — Spend buff   : enemy_nab > 0           → spend it     (95/5/0)
--   Rule 3 — Counter read : player defended last turn → shift 35% of
--                           attack weight to focus (opponent builds power
--                           instead of wasting an attack into a block)


-- ─── 1. battle_pick_enemy_action ─────────────────────────────────────────────
...
begin
  ...
  -- Deterministic roll seeded by run+round (same game always plays the same)
...
-- ─── 2. submit_battle_action — call state-aware helper ───────────────────────
-- Only the enemy action picker block changes; everything else is identical
-- to 016/017.
