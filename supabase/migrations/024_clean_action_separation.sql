-- 024_clean_action_separation.sql
-- Clarifies action identities for both player and enemy:
--
--   DEFEND  → pure damage reduction only (removes +10% next_attack_bonus grant)
--   FOCUS   → flat +40% next_attack_bonus (replaces opaque momentum_boost +20%)
--
-- Design intent:
--   Attack  = deal damage now
--   Defend  = prevent damage this turn (situational read)
--   Focus   = sacrifice a turn of safety for a guaranteed +40% on the next hit
--
-- momentum_boost columns are preserved in schema/payload but no longer written;
-- the damage formula's (1 + momentum_boost) factor stays correct at 1.0.
-- next_attack_bonus is now the sole carry-over buff, set only by Focus.
...
begin
  ...
  -- ══════════════════════════════════════════════════════════════════════════
  -- ACTION CASES (shared between first-actor and second-actor blocks)
  -- ══════════════════════════════════════════════════════════════════════════
  ...
  if v_player_goes_first then
    ...
    case v_player_action
      ...
      when 'defend' then
        -- Pure damage reduction: no carry-over buff granted
        v_log_entry := jsonb_build_object(
          'id', gen_random_uuid(), 'round', v_current_round, 'phase', 'action',
      ...
      when 'focus' then
        -- Flat +40% on next attack via next_attack_bonus (replaces momentum_boost path)
        v_player_next_attack_bonus := 0.40;
        v_log_entry := jsonb_build_object(
          'id', gen_random_uuid(), 'round', v_current_round, 'phase', 'action',
          ...
          'crit', false, 'defended', false,
          'consumed_momentum_boost', false, 'consumed_next_attack_bonus', false,
          'message', v_companion_name || ' charges up for a powerful strike!'
        );
    end case;
    v_new_log := v_new_log || v_log_entry;
    ...
    case v_enemy_action
      ...
      when 'focus' then
        -- Flat +40% on next attack
        v_enemy_next_attack_bonus := 0.40;
        v_log_entry := jsonb_build_object(
          'id', gen_random_uuid(), 'round', v_current_round, 'phase', 'action',
          ...
          'crit', false, 'defended', false,
          'consumed_momentum_boost', false, 'consumed_next_attack_bonus', false,
          'message', v_opponent.name || ' charges up for a powerful strike!'
        );
    end case;
    v_new_log := v_new_log || v_log_entry;
